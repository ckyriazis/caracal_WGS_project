...
# 10 generations ~ 10/(4000*2) # 0.001
def twoEpochFixedT(params, ns, pts): 
    ...
    phi = Integration.one_pop(phi, xx, 0.0008, nu)  # contraction for set duration of ~10 generations 
...
upper_bound = [10]
lower_bound = [1e-4]
p0 = [0.01] # initial parameters


func=twoEpochFixedT # set the function
...
scaled_param_names=("Nanc_FromTheta_scaled_dip","nu_scaled_dip")
scaled_popt=(Nanc,nu_scaled_dip,nu_scaled_dip)
